    ...
    )
{
  ...
  if (argc != 3)
  {
    cerr << "Usage: " << argv[0] << " <server_url> <rpd>" << endl;
    exit(EXIT_FAILURE);
  }
  
  std::string server_url( argv[1] );
  const int rpd = from_string<int>( argv[2] );

  std::string system_str = "\
init 1000 ATP\n\
init 50 Prot(type~Enz)\n\
init 200 aa\n\
init 1 Cit\n\
Cit, Prot(type~Enz), ATP -> Cit, Prot(type~Enz), Lip, desechos [1]\n\
Cit, Prot(type~Enz) -> Cit, Prot(type~Enz), ATP, desechos [1]\n\
Cit, Rib, TEnz, mRNA, ATP, aa, aa, aa, aa, aa, aa, aa, aa, aa, aa -> Cit, Rib, TEnz, Prot(type~Enz) [1]\n\
Cit, Rib, TEst, mRNA, ATP, aa, aa, aa, aa, aa, aa, aa, aa, aa, aa -> Cit, Rib, TEst, Prot(type~Est) [1]\n\
Nucleo, ATP -> Nucleo, mRNA [1]\n\
Mit, Prot(type~Enz) -> Mit, Prot(type~Enz), ATP, ATP, ATP, ATP, ATP, ATP, ATP, ATP, ATP, ATP, desechos, desechos [1]\n\
Lis, Prot(type~Enz), ATP, PU -> Lis, Prot(type~Enz), aa, aa, aa, aa, aa, aa, aa, aa, aa, aa, desechos [1]\n\
Lis, Prot(type~Enz), ATP, LU -> Lis, Prot(type~Enz), desechos [1]\n\
Per, Prot(type~Enz), RL -> Per, Prot(type~Enz) [1]\n\
REL, Prot(type~Enz), ATP -> REL, Prot(type~Enz), Lip, Lip, Lip, Lip, Lip, desechos, desechos [1]\n\
RER, Rib, TDef, Prot(type~Enz), mRNA, ATP, aa, aa, aa, aa, aa, aa, aa, aa, aa, aa -> RER, Rib, TDef, Prot(type~Enz), Prot(type~Def) [1]\n\
RER, Golgi, Rib, TTyC, Prot(type~Enz), mRNA, ATP, aa, aa, aa, aa, aa, aa, aa, aa, aa, aa -> RER, Golgi, Rib, TTyC, Prot(type~Enz), Prot(type~TyC) [1]\n";

  system s = read_system<LinearGillespie, RuleSet>(system_str);
